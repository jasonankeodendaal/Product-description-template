...
export const AuthModal: React.FC<AuthModalProps> = ({ onUnlock, userPin }) => {
  ...
  const handlePinChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.replace(/[^0-9jJ]/gi, ''); // Allow numbers and 'j', case-insensitive
    if (value.length <= 5) {
      setPin(value);
      setError('');
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (pin.toLowerCase() === CREATOR_PIN.toLowerCase()) {
        onUnlock('creator');
    } else if (userPin && pin.toLowerCase() === userPin.toLowerCase()) {
        onUnlock('user');
    } else {

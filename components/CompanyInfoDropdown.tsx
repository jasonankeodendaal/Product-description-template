    ...
    >
      <div className="p-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-md font-bold text-[var(--theme-orange)]">{settings.companyName}</h2>
        ...
        {(settings.tel || settings.email || settings.website) && (
          <div className="space-y-3 mt-4 border-t border-[var(--theme-border)]/50 pt-4">
            {settings.tel && (
              <a href={`tel:${settings.tel}`} className="flex items-center gap-3 text-sm text-[var(--theme-text-secondary)] hover:text-[var(--theme-orange)] transition-colors">
                <PhoneIcon /> <span>{settings.tel}</span>
              </a>
            )}
            {settings.email && (
              <a href={`mailto:${settings.email}`} className="flex items-center gap-3 text-sm text-[var(--theme-text-secondary)] hover:text-[var(--theme-orange)] transition-colors">
                <MailIcon /> <span>{settings.email}</span>
              </a>
            )}
            {settings.website && (
              <a href={settings.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-[var(--theme-text-secondary)] hover:text-[var(--theme-orange)] transition-colors">

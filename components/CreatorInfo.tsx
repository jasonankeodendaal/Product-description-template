            ...
            <div className="flex flex-col items-center -mt-20">
             ...
             <div className="space-y-3 mt-6 border-t border-[var(--theme-border)] pt-6 text-left">
                {settings.creator.tel && (
                    <a href={`tel:${settings.creator.tel}`} className="flex items-center gap-4 p-3 rounded-lg hover:bg-[var(--theme-bg)]/50 transition-colors">
                        <PhoneIcon /> <span className="text-[var(--theme-text-primary)]">{settings.creator.tel}</span>
                    </a>
                )}
                {settings.creator.email && (
                    <a href={`mailto:${settings.creator.email}`} className="flex items-center gap-4 p-3 rounded-lg hover:bg-[var(--theme-bg)]/50 transition-colors">
                        <MailIcon /> <span className="text-[var(--theme-text-primary)]">{settings.creator.email}</span>
                    </a>
                )}
                {settings.creator.whatsapp && (
                    <a href={settings.creator.whatsapp.startsWith('http') ? settings.creator.whatsapp : `https://wa.me/${settings.creator.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 rounded-lg hover:bg-[var(--theme-bg)]/50 transition-colors">
                        <WhatsappIcon className="h-5 w-5 text-[var(--theme-text-secondary)]" /> <span className="text-[var(--theme-text-primary)]">WhatsApp 1</span>
                    </a>
                )}
                {settings.creator.whatsapp2 && (
                    <a href={settings.creator.whatsapp2.startsWith('http') ? settings.creator.whatsapp2 : `https://wa.me/${settings.creator.whatsapp2.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 rounded-lg hover:bg-[var(--theme-bg)]/50 transition-colors">
                        <WhatsappIcon className="h-5 w-5 text-[var(--theme-text-secondary)]" /> <span className="text-[var(--theme-text-primary)]">WhatsApp 2</span>
                    </a>
                )}
             </div>
            
            <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 text-[var(--theme-text-secondary)]/50 hover:text-[var(--theme-text-primary)]" aria-label="Close">
              <XIcon />
            </button>
          </div>
        </div>
